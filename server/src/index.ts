...
try {
    ...
    // Ensure upload directory exists - REMOVED as files are now stored in GridFS
    // const uploadDir = path.join(__dirname, '../uploads');
    // console.log('Upload directory path:', uploadDir);
    // try {
    //     if (!fs.existsSync(uploadDir)) {
    ...
    // Database Connection
    const prompt = process.env.MONGO_URI || 'mongodb://localhost:27017/pdf-form-system';
    const connect = mongoose.connect(prompt);
